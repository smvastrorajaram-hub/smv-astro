...
async function loadDashboard(expectedRole=null,force=false){
 ...
 try{
  if(box && active() && (!box.children.length || !dashboardReadyUid || dashboardReadyUid!==loadUid)) box.innerHTML='<div class="card"><div class="small">Loading your dashboard...</div></div>';
...
if(auth){ onAuthStateChanged(auth,async user=>{
   ...
   if(user){
     ...
     // Fast role resolution: normal Customer/Astrologer login must never wait for
     // the protected Render Admin endpoint. Resolve Firebase profile + astrologer
     // document in parallel; only the configured Admin UID/profile enters Admin.
     let headerRole='';
     try{
       const cached=JSON.parse(sessionStorage.getItem('smv_login_role')||'null');
       if(cached?.uid===user.uid) headerRole=String(cached.role||'').toLowerCase();
     }catch(_e){}
     if(!headerRole){
       const [hp,ha]=await Promise.all([
         withTimeout(getDoc(doc(db,'smv_users',user.uid)),8000).catch(()=>null),
         withTimeout(getDoc(doc(db,'smv_astrologers',user.uid)),8000).catch(()=>null)
       ]);
