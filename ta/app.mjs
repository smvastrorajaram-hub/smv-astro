...
async function renderApi(path, options={}, userOverride=null){
 ...
 const task=(async()=>{
  ...
  try{
   ...
   if(!response.ok){
    if(response.status===404&&!data.error)throw new Error('Backend API கிடைக்கவில்லை (404). Render-ல் புதிய server.js மற்றும் refund-service.js இரண்டையும் deploy செய்து backend URL சரியா எனச் சரிபார்க்கவும்.'+' '+path);
...
async function retryCustomerPayment(questionId, triggerButton){
  ...
  if(btn){btn.disabled=true;btn.textContent='கட்டணம் தயாராகிறது…';}
  try{
    ...
    const options={
      ...
      handler:async function(response){
        if(btn){btn.disabled=true;btn.textContent='கட்டணம் உறுதிப்படுத்தப்படுகிறது…';}
        try{
...
const userStatus=String(
  ...
  return;
   ...
   let availableQuestions=qs.docs.filter(d=>{const q=d.data()||{};return q.astrologerId===currentUser.uid&&!!q.adminQuestionApprovedAt&&['assigned_to_astrologer','available_to_astrologers','reallocated'].includes(String(q.allocationStatus||''))&&['admin_approved','paid'].includes(String(q.status||''));}).map(d=>({id:d.id,...d.data()}));
   try{
...
<div class="card" style="margin-top:16px"><h3>கட்டண முறை</h3><p class="small">Your bank/UPI details are private. Full details are not displayed again.</p><button class="btn gray" id="changePayoutBtn2">கட்டண முறையை மாற்றவும்</button></div>`;
  ...
  try{
   ...
   document.querySelectorAll('[data-edit-final-answer]').forEach(b=>b.onclick=async()=>{
     const questionId=b.dataset.editFinalAnswer; b.disabled=true; b.textContent='திறக்கப்படுகிறது...';
     try{const r=await renderApi('/astrologer/edit-answer',{method:'POST',body:JSON.stringify({questionId})});if(!r?.success)throw new Error(r?.error||'இந்த பதிலைத் திருத்த முடியவில்லை.');await loadDashboard('astrologer',true);requestAnimationFrame(()=>setTimeout(()=>{const ta=$('ans_'+questionId);(ta||$('astroQuestionQueue'))?.scrollIntoView({behavior:'smooth',block:'center'});ta?.focus();},100));}
     catch(e){alert(e.message||String(e));b.disabled=false;b.textContent='பதிலைத் திருத்து';}
   });
...
function smvRenderOpenWorkflowControl(data){
 ...
 card.innerHTML=`<h3>கேள்வி / பதில் அனுமதி கட்டுப்பாடு</h3><p class="small"><b>ALLOW</b> ON ஆக இருந்தால், பணம் செலுத்தப்பட்ட கேள்விகள் நிர்வாகி ஒப்புதல் இல்லாமல் அனைத்து அங்கீகரிக்கப்பட்ட ஜோதிடர்களின் திறந்த கேள்விகள் பகுதியில் தெரியும். முதலில் Claim செய்கிற ஜோதிடருக்கே கேள்வி ஒதுக்கப்படும். அவர் சமர்ப்பிக்கும் பதில் நிர்வாகி ஒப்புதல் இல்லாமல் வாடிக்கையாளருக்குச் செல்லும். வாடிக்கையாளர் பதிலை பார்க்கும் வரை மட்டும் ஜோதிடர் திருத்தலாம்.</p><div class="action-row"><button class="btn ${on?'':'gray'}" id="smvWorkflowAllow" type="button">${on?'ALLOW ON':'ALLOW OFF'}</button><span class="small"><b>தற்போதைய முறை:</b> ${on?'திறந்த கேள்விகள் + நேரடி பதில்':'நிர்வாகி கேள்வி/பதில் ஒப்புதல் தேவை'}</span></div><div id="smvWorkflowMsg" class="small"></div>`;
 $('smvWorkflowAllow').onclick=async()=>{const b=$('smvWorkflowAllow'),next=!on;b.disabled=true;try{const r=await renderApi('/admin/set-open-workflow',{method:'POST',body:JSON.stringify({allowWithoutAdminApproval:next})});if(!r?.success)throw new Error(r?.error||'Workflow மாற்ற முடியவில்லை.');$('smvWorkflowMsg').innerHTML='<span class="success">சேமிக்கப்பட்டது.</span>';await loadAdminPanel();}catch(e){$('smvWorkflowMsg').innerHTML='<span class="error">'+escapeHtml(e.message||String(e))+'</span>';b.disabled=false;}};
}
